...
\caption{{\small \textsf{RMSE} statistics derived a 25--member {\enkf} simulation for $\mathsf{m} = 10$ showing the difference between the \textsf{WCDA} and the \textsf{SCDA} implementation. The model is spun up to stability during time $t \in [0, 100]$ whereas the {\rmse} statistics are calculated for time $t \in [550, 1100]$. In this particular experiment, \textsf{WCDA} seems to perform better than \textsf{SCDA}; this is expected due to the presence of stochastic forcing term whose cross-section is of the same order as model and observation errors.}}
\label{fig:6}
\end{figure}
\begin{multicols}{2}
