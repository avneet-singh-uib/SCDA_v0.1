...
\caption{{\small \textsf{RMSE} statistics derived a 25--member {\enkf} simulation as a function of  `$\mathsf{m}$' (\tit{vertical axes}) and $\tau_\mathsf{win}$ (\tit{horizontal axes}) showing the difference between the \textsf{WCDA} and the \textsf{LACC} implementation. Top panel shows the oceanic component while the bottom panel depicts the atmosphere.}}
\label{fig:7}
\end{figure}
